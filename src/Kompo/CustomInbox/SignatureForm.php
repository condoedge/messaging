...
class SignatureForm extends Form
{
    ...
    {
        return _Html($this->model->toHtml());
    }
    ...
    public function render()
    {
        return _Rows(
            ...
            /*_Html('messaging-signature-preview')->class('text-sm font-semibold text-level3 mb-2'),
            _Panel(
                $this->signaturePreview()
            )->id('signature-apercu-panel')
            ->class('border border-gray-300 rounded-lg p-2 mb-4 overflow-x-auto'),*/
            _SubmitButton('Save')->inPanel('signature-apercu-panel')
